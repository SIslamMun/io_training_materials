...
On this page
# Pipeline Tests
Pipeline tests are used to run experiment sets using a grid search.
## Example File[​](https://grc.iit.edu/docs/jarvis/jarvis-cd/pipeline-tests/#example-file "Direct link to Example File")
Below is an example of a pipeline for running various configurations of spark KMeans.
```
config:  
...
## config:[​](https://grc.iit.edu/docs/jarvis/jarvis-cd/pipeline-tests/#config "Direct link to config:")
This section is the skeleton of a pipeline. It has the same exact parameters as a [pipeline script](https://grc.iit.edu/docs/jarvis/jarvis-cd/pipeline-scripts).
This example, the pipeline will be called mm_kmeans_spark and launch a spark cluster, dataset generator, and spark kmeans in that order.
## vars:[​](https://grc.iit.edu/docs/jarvis/jarvis-cd/pipeline-tests/#vars "Direct link to vars:")
Each pkg in the pipeline has a set of variables it exposes. In this example, we vary the dataset size, a window size, and the number of nodes in the spark cluster.
The syntax of variables are: `pkg_name.var_name`
## loop:[​](https://grc.iit.edu/docs/jarvis/jarvis-cd/pipeline-tests/#loop "Direct link to loop:")
This is represents what test loop should look like. In pseudocode, the above loop would translate to python roughly as follows:
```
...
By having a separate loop section, you can define certain variables as together or independent to reduce the number of total test cases. In this example, `mm_kmeans_df.window_size` and `mm_kmeans_df.df_size` vary together, but independently from `spark_cluster.num_nodes`.
`mm_kmeans_df.window_size` and `mm_kmeans_df.df_size` must have the same size (in this case 6).
# repeat:
The number of times each experiment should be conducted. For example, this can be used to calculate the average across experiment runs to get a better understanding of variability and noise in your study.
In this example, experiments are only conducted once.
# output
This is the directory where the results are stored. Note, jarvis stores the pipeline's shared directory, private directory, and configuration directory in the following three environment variables: `${SHARED_DIR}`, `${PRIVATE_DIR}`, and `${CONFIG_DIR}`.
...
To get more columns, pkgs can define a custom `_get_stat()` function. This is more for developers than users: Below is an example of a custom stat for the YCSB benchmark, which analyzes the output of YCSB for its throughput and total runtime.
```
class Ycsb:  
...
[Previous Pipeline Scripts](https://grc.iit.edu/docs/jarvis/jarvis-cd/pipeline-scripts)[Next Pipeline Indexes](https://grc.iit.edu/docs/jarvis/jarvis-cd/pipeline-indexes)
